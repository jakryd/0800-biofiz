...
parser = argparse.ArgumentParser(
    usage="python %(prog)s [options]", description="Implements pi estimators."
)
...
px_hist = np.array(
    [np.random.normal(mu_target, sigma_target) for _ in range(n_samples)]
)
qx_hist = np.array(
    [np.random.normal(mu_approx, sigma_approx) for _ in range(n_samples)]
)
...
plt.figure()
plt.hist(
    function(px_hist), bins=100, alpha=0.5, label="$f(x)$ from $p(x)$", density=True
)
plt.hist(
    function(qx_hist), bins=100, alpha=0.5, label="$f(x)$ from $q(x)$", density=True
)
plt.xlabel("$f(x)$")
plt.ylabel("distribution")
plt.legend()
