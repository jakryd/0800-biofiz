...
class MC_Object:
    ...
    def random_position(self):
        return np.array(
            [
                np.random.uniform(
                    self.potential.grid_min[i], self.potential.grid_max[i]
                )
    ...
    def init_potential(self):
        ...
        hist = np.array(
            [
                np.linspace(
                    self.potential.grid_min[i], self.potential.grid_max[i], self.bins[i]
                )
                for i in range(self.dimension)
            ]
        )
        ...
        for i in range(self.bins[0]):
            for j in range(self.bins[1]):
                self.pot_grid[i, j], _ = self.potential.eval(
                    [self.x_grid[i, j], self.y_grid[i, j]]
                )
    ...
    def mc_step(self):
        ...
        delta_pot = self.potential.eval(pos)[0] - self.potential.eval(curr_pos)[0]
    ...
    def plot(self):
        ...
        cf = ax.contourf(
            self.x_grid,
            self.y_grid,
            self.pot_grid - self.pot_grid.min(),
            vmin=0.0,
            vmax=50.0,
...
parser.add_argument(
    "--step_size", type=float, default=0.1, help="step size", required=False
)
args = parser.parse_args()

mb_potential = MuellerBrown()
engine = MC_Object(potential=mb_potential, temperature=args.temp, step=args.step_size)
