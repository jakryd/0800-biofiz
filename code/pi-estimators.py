...
def pi_estimator(n_samples):
    ...
    for i in range(n_samples):
        ...
        if dist <= 1.0:
            n_inside += 1
        n_all += 1
        est.append(4.0 * float(n_inside) / float(n_all))
    return np.array(est)
...
def pi_estimator_markov_chain(n_samples, step=0.1):
    ...
    for i in range(n_samples):
        ...
        n_all += 1
        est.append(4.0 * float(n_inside) / float(n_all))
    acc_ratio = float(n_accepted) / float(n_all)
    return np.array(est)
...
plt.fill_between(
    x=range(n_samples), y1=mean - std, y2=mean + std, label=r"mean $\pm$ std"
)
plt.axhline(y=np.pi, color="k", linestyle="--", label=r"$\pi$")
