...
class MuellerBrown:
    ...
    saddle = np.array([[-0.822, 0.624], [-0.212, 0.293]])
    ...
    def eval(self, position):
        ...
        for i in range(4):
            ...
            force[0] += (
                -self.A[i]
                * (2.0 * self.a[i] * (x - self.x0[i]) + self.b[i] * (y - self.y0[i]))
                * tmp
            )
            force[1] += (
                -self.A[i]
                * (self.b[i] * (x - self.x0[i]) + 2.0 * self.c[i] * (y - self.y0[i]))
                * tmp
            )
